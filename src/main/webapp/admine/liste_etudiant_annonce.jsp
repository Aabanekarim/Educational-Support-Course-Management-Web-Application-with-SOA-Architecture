...
com.admine ad =(com.admine)sess.getAttribute("admine");
	...
	if(request.getParameter("ida")!=null){
		...
		ida =(int)sess.getAttribute("idae");
	}
	sess.setAttribute("idae", ida);%>
              ...
              <table class="table datatable">
                <thead>
                  <tr>
                    ...
                    <th scope="col"><h3>paiment</h3></th>
                  </tr>
                </thead>
                <tbody>
                <%

                ArrayList l=(ArrayList)s.listetuannonce(ida);
...
for (int i = start2; i < end2; i++) {
	com.etu e =(com.etu)l.get(i);
                %>
                  <tr>
                    ...
                    <td>
                    <%if(s.getpayement(e.getId(), ida)==0){ %>
                    <a href="../traiter?fval=paiment&ida=<%=ida %>&ide=<%=e.getId()%>" style="text-decoration: none;" ><button type="button" class="btn btn-danger rounded-pill" >payer</button></a>
                    <%} else {%>
                    <h3>est payee</h3>
                    <%} %>
                    </td>
  ...
  <div class="pagination">
    <form action="liste_etudiant_annonce.jsp" >
      <button name="currentPage2" value="<%= currentPage2 - 1 %>" <% if (currentPage2 <= 1) out.print("disabled"); %> class="btn-previous" onclick="storeScrollPosition()"><h3>Précédent</h3></button>
      <button name="currentPage2" value="<%= currentPage2 + 1 %>" <% if (end2 >= l.size()) out.print("disabled"); %> class="btn-next" onclick="storeScrollPosition()"><h3>Suivant</h3></button>
    </form>
  </div>
            </div>
          </div>
