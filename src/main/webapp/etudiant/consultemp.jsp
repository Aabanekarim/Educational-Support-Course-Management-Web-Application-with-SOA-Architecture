...
<div class="box">
                ...
                <thead>
                  <tr>
                    ...
                    <th ><h1>Lundi</h1></th>
                    <th ><h1>Mardi</h1></th>
                    <th ><h1>Mercredi</h1></th>
                    <th ><h1>Jeudi</h1></th>
                    <th ><h1>Vendredi</h1></th>
                    <th ><h1>Samedi</h1></th>
                  </tr>
                </thead>
                <tbody>
                <% for(annonce e : s.monannonce(u.getId())){
			  %>
                  <tr>
                    <th ><h1> <%=e.getJour() %></h1></th>
                    <td><h3><%if(e.getSeance().contains("lundi")){ %><%=e.getMatier() %><%} %></h3></td>
                    <td><h3><%if(e.getSeance().contains("mardi")){ %><%=e.getMatier() %><%} %></h3></td>
                    <td><h3><%if(e.getSeance().contains("mercredi")){ %><%=e.getMatier() %><%} %></h3></td>
                    <td><h3><%if(e.getSeance().contains("jeudi")){ %><%=e.getMatier() %><%} %></h3></td>
                    <td><h3><%if(e.getSeance().contains("vandredi")){ %><%=e.getMatier() %><%} %></h3></td>
                    <td><h3><%if(e.getSeance().contains("samedi")){ %><%=e.getMatier() %><%} %></h3></td>
