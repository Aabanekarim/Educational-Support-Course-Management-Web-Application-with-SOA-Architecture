...
<% HttpSession sess=request.getSession(); 
	...
	int idp=-1;
	if(id==null){
		 idp = (int) sess.getAttribute("idp");
	}else{
		 idp = Integer.parseInt(id);
	}
	com.prof p = s.getprof(idp);
	sess.setAttribute("idp", idp);
	com.admine ad=(com.admine)sess.getAttribute("admine");
sess.setAttribute("fval", "modifierprof");
...
<section class="form-container">
   ...
   <form action="../traiter" method="post" >
      ...
      <p>Nom</p>
      <input type="text" name="nom" placeholder="entrer la matiere" value="<%=p.getNom() %>" maxlength="50" class="box" required="required">
      <p>Prenom</p>
      <input type="text" name="prenom" placeholder="entrer la matiere" value="<%=p.getPrenom() %>" maxlength="50" class="box" required="required">
      <p>Email</p>
      <input type="text" name="email" placeholder="entrer la filiere" value="<%=p.getEmail() %>" maxlength="50" class="box" required="required">
      <p>Mot de pass</p>
      <input type="password" name="pass" placeholder="entrer la filiere" value="<%=p.getPass() %>" maxlength="50" class="box" required="required">
      <p>Confirmer Mot de pass</p>
      <input type="password" name="pass2" placeholder="entrer la filiere" value="<%=p.getPass() %>" maxlength="50" class="box" required="required">
      
                      
      <input type="submit" value="modifierprof" name="fval" class="btn">
      <%if(testpass!=null){ %>
                <p><%=testpass %></p>
                <%} %>
                <%sess.setAttribute("testpass", null); %>
   </form>
